...
arial_dir <- "/home/dengsc/.fonts/truetype/msttcorefonts"
if (file.exists(file.path(arial_dir, "ARIAL.TTF"))) {
  arial_files <- c(
    regular = file.path(arial_dir, "ARIAL.TTF"),
    bold = file.path(arial_dir, "ARIALBD.TTF"),
    italic = file.path(arial_dir, "ARIALI.TTF"),
    bolditalic = file.path(arial_dir, "ARIALBI.TTF")
  )
} else {
  liberation_dir <- "/usr/share/fonts/truetype/liberation2"
  arial_files <- c(
    regular = file.path(liberation_dir, "LiberationSans-Regular.ttf"),
    bold = file.path(liberation_dir, "LiberationSans-Bold.ttf"),
    italic = file.path(liberation_dir, "LiberationSans-Italic.ttf"),
    bolditalic = file.path(liberation_dir, "LiberationSans-BoldItalic.ttf")
  )
}

font_add(
  family = "Arial",
  regular = arial_files[["regular"]],
  bold = arial_files[["bold"]],
  italic = arial_files[["italic"]],
  bolditalic = arial_files[["bolditalic"]]
)
...
# 仅绘制主要细胞类型；不再拼接左侧 seurat cluster 图
celltype_only_dir <- "/public3/DSC/single_cell/Result/figer_new/UMAP_celltype_only_from_main"
dir.create(celltype_only_dir, showWarnings = FALSE, recursive = TRUE)
...
# 让 showtext 的字体尺寸与高分辨率 PNG 的 300 dpi 保持一致
showtext_opts(dpi = 300)

ggplot2::ggsave(
  filename = file.path(celltype_only_dir, "F1.1_UMAP_celltype_only.png"),
  plot = p_celltype,
  width = 13,
  height = 8.5,
  dpi = 300
...
ggplot2::ggsave(
  filename = file.path(celltype_only_dir, "F1.1_UMAP_celltype_only.pdf"),
  plot = p_celltype,
  width = 13,
  height = 8.5,
  device = cairo_pdf
...
# ==============================================================================
# 重新绘制去批次效应评估图：每个公共队列单独一个 UMAP 面板
# ==============================================================================
batch_levels <- c(
  "GSE131778", "GSE155468", "GSE159677",
  "GSE216860", "GSE224273", "GSE234077",
  "GSE247238", "GSE253903", "GSE260657"
)
batch_colors <- c(
  "GSE131778" = "#F8766D",
  "GSE155468" = "#D89000",
...
umap_batch_df <- as.data.frame(Embeddings(data, reduction = "umap"))
colnames(umap_batch_df)[1:2] <- c("UMAP_1", "UMAP_2")
umap_batch_df$Batch <- factor(
  data$GSE_Clean_ID,
  levels = intersect(batch_levels, unique(data$GSE_Clean_ID))
)

set.seed(123)
umap_batch_df <- umap_batch_df[sample(seq_len(nrow(umap_batch_df))), ]

p3_batch_clean <- ggplot(
  umap_batch_df,
  aes(x = UMAP_1, y = UMAP_2, color = Batch)
) +
  geom_point(size = 0.65, alpha = 0.9, shape = 16, stroke = 0) +
  facet_wrap(~Batch, ncol = 3, drop = TRUE) +
  scale_color_manual(values = batch_colors, drop = FALSE) +
  scale_x_continuous(expand = expansion(mult = 0.04)) +
  scale_y_continuous(expand = expansion(mult = 0.04)) +
  coord_fixed() +
  theme_dr(xlength = 0.18, ylength = 0.18) +
  labs(
    title = "Public cohorts batch correction",
    x = "UMAP_1",
    y = "UMAP_2"
  ) +
  guides(color = "none") +
  theme(
    ...
    strip.text = element_text(size = 22, face = "bold", family = "Arial"),
...
batch_split_dir <- "/public3/DSC/single_cell/Result/figer_new/Batch_UMAP_split_from_main"
dir.create(batch_split_dir, showWarnings = FALSE, recursive = TRUE)

showtext_opts(dpi = 300)
ggplot2::ggsave(
  filename = file.path(batch_split_dir, "F1.2_UMAP_Batch_split.png"),
  plot = p3_batch_clean,
  width = 18,
  height = 17,
  dpi = 300
...
ggplot2::ggsave(
  filename = file.path(batch_split_dir, "F1.2_UMAP_Batch_split.pdf"),
  plot = p3_batch_clean,
  width = 18,
  height = 17,
  device = cairo_pdf
...
p_celltype <- DimPlot(sub_integrated_data,
  ...
  theme(
        ...
        plot.margin = margin(t = 15, r = 24, b = 15, l = 15)) +
  NoLegend() +
  coord_flip(clip = "off")
...
mps_celltype_only_dir <- "/public3/DSC/single_cell/Result/figer_new/MPS_celltype_only_from_main"
dir.create(mps_celltype_only_dir, showWarnings = FALSE, recursive = TRUE)

print(p_celltype)
showtext_opts(dpi = 300)
ggplot2::ggsave(
  filename = file.path(mps_celltype_only_dir, "S1.1_MPS_UMAP_celltype_only.png"),
  plot = p_celltype,
  width = 10,
  height = 8.5,
  dpi = 300
...
ggplot2::ggsave(
  filename = file.path(mps_celltype_only_dir, "S1.1_MPS_UMAP_celltype_only.pdf"),
  plot = p_celltype,
  width = 10,
  height = 8.5,
  device = cairo_pdf
...
ggplot2::ggsave(p_selected_cells,
                ...
                device = cairo_pdf)
ggplot2::ggsave(p_selected_cells,
                filename = '/public3/DSC/single_cell/Result/figer_new/monocle3/F2.7_MM_selected_cells.png',
                width = 18,
                height = 8,
                dpi = 300,
                bg = "white")
